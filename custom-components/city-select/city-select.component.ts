...
import { tagAnimation } from 'ng-zorro-antd/src/core/animation/tag-animations';
...
export class CitySelectComponent implements OnInit, AfterViewInit, ControlValueAccessor {
    ...
    deleteChoice(obj, $event) {
        $event.stopPropagation();
        this.totalTree.forEach((p, i) => {
            if ((<any>this.checkedProArr).includes(p.code)) {
                //如果删除省
                if (obj.code === p.code) {
                    p.click = false;
                    //清除省市区checked
    ...
    itemsIsChecked(type, item) {
        switch (type) {
            case 'province':
                return (<any>this.checkedProArr).includes(item.code);
            case 'city':
