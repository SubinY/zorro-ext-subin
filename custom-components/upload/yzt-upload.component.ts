...
export class YztUploadComponent implements OnInit {
    ...
    @Output()
    fileListChange: EventEmitter<YztUploadFile[]> = new EventEmitter<YztUploadFile[]>();
    ...
    getYztUploadFiles(files: YztUploadFile[] = this._fileList): YztUploadFile[] {
        ...
        files.forEach(file => {
            ...
            if (!file.id) {
                ...
                if ((file.response && file.response[0])) {
                    ...
                    file.url = file.response[0].url || file.url;
                }
                file.message = file.id ? '上传失败' : ''
